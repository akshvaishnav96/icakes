...
@section('main-section')
    <div class="mx-auto w-full max-w-[550px] bg-white my-5">
        ...
        <form class="py-6 px-9" action="{{ $url }}" method="post">
            ...
            @if ($errors->has('err'))
                <x-input label="enter size name" id="cake_sizes" type="text" name="size_name" placeholder="enter sizename"
                    valuetext="{{ old('sizename') }}" redClass="border-red-500" />
            @else
                <x-input label="enter size name" id="cake_sizes" type="text" name="size_name"
                    placeholder="enter sizename" valuetext="{{ isset($oldsize) ? $oldsize : '' }}"
            ...
            <x-input label="size_description" id="size_description" type="text" name="description"
                placeholder="size description " valuetext="{{ isset($description) ? $description : old('description') }}"
                redClass="{{ $errors->has('description') ? 'border-red-500' : 'border-red-100' }}" />
            @error('description')
        ...
        @foreach ($size as $key => $size)
            <div
                ...
                <div class=" flex items-center justify-between" id="showsizes">
                    <div class="flex items-center">

                        <div class="flex flex-col ml-3">
                            <a class="text-xl font-semibold text-gray-800"></a>
                            <div class="font-medium leading-none">size Name: {{ $size->size_name }}</div>
                            <div class="font-medium leading-none">description: {{ $size->description }}</div>
                            </p>
                        </div>
                    </div>
                    <a href ="/size/edit/{{ $size->size_id }}"><button id ="uplodesizeEditBtn"
                    ...
                    <a href = "/size/{{ $size->size_id }}"> <button id ="uplodesizeDelete Btn"
