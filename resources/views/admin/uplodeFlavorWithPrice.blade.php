...
@push('title')
    <title>cake flavor with price</title>
@endpush
...
@section('main-section')
    <div class="mx-auto w-full max-w-[550px] bg-white my-5">
        ...
        <form class="py-6 px-9" action="{{ $url }}" method="post">
            ...
            @isset($flavors)
                <label class="mb-5 block text-xl font-semibold text-[#07074D]">
                    Select Flavor<span class="text-red-500">*</span>
                    <select
                        ...
                        @foreach ($flavors as $flavor)
                            <option
                                ...
                                @isset($old_flavor_name){{ $old_flavor_name === $flavor['flavor_name'] ? 'selected' : '' }} @endisset>
            ...
            @if ($errors->has('err'))
                <x-input label="enter_flavor_price" id="enter_flavor_with_price" type="text" name="flavor_price"
                    placeholder="enter price" valuetext="{{ old('flavor_price') }}" redClass="border-red-500" />
            @else
                <x-input label="enter_flavor_price" id="enter_flavor_with_price" type="text" name="flavor_price"
                    placeholder="enter price" valuetext="{{ isset($old_flavor_price) ? $old_flavor_price : '' }}"
        ...
        @foreach ($flavorprice as $key => $flavors)
            <div
                ...
                <div class=" flex items-center justify-between" id="showsubcategorys">
                    <div class="flex items-center">
                        ...
                        <div class="flex flex-col ml-3">
                            ...
                            <div class="font-medium  leading-none">Flavor name:
                                {{ $flavors['flavor_name'] }}
                            </div>
                            <div class="font-medium leading-none text-red-500 my-2">Price : {{ $flavors['flavor_price'] }}
                            </div>
                            </p>
                        </div>
                    </div>
                    <a href ="/flavorprice/edit/{{ $flavors['flavor_with_prices_id'] }}"><button
                    ...
                    <a href = "/flavorprice/{{ $flavors['flavor_with_prices_id'] }}"> <button
