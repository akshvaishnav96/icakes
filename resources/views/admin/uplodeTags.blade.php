...
@section('main-section')
    <div class="mx-auto w-full max-w-[550px] bg-white my-5">
        ...
        <form class="py-6 px-9" action="{{ $url }}" method="post">
            @csrf
            @if ($errors->has('err'))
                <x-input label="enter tag name" id="cake_tags" type="text" name="tag_name" placeholder="enter tagname"
                    valuetext="{{ old('tagname') }}" redClass="border-red-500" />
            @else
                <x-input label="enter tag name" id="cake_tags" type="text" name="tag_name" placeholder="enter tagname"
                    valuetext="{{ isset($oldtag) ? $oldtag : '' }}" redClass="border-red-100" />
            @endif
            @error('err')
        ...
        @foreach ($tag as $key => $tag)
            <div
                ...
                <div class=" flex items-center justify-between" id="showtags">
                    <div class="flex items-center">

                        <div class="flex flex-col ml-3">
                            <a class="text-xl font-semibold text-gray-800"></a>
                            <div class="font-medium leading-none">Tag Name: {{ $tag->tag_name }}</div>
                            </p>
                        </div>
                    </div>
                    <a href ="/tags/edit/{{ $tag->tag_id }}"><button id ="uplodeTagEditBtn"
                    ...
                    <a href = "/tags/{{ $tag->tag_id }}"> <button id ="uplodeTagDelete Btn"
