...
@push('title')
    <title>Cake Size with Price</title>
@endpush
...
@section('main-section')
    <div class="mx-auto w-full max-w-[550px] bg-white my-5">
        ...
        <form class="py-6 px-9" action="{{ $url }}" method="post">
                    ...
                    <select
                        ...
                        @foreach ($tier as $tier)
                            <option
                                ...
                                value="{{ $tier['tier_id'] }}"
                                @isset($old_tier_name){{ $old_tier_name === $tier['tier_name'] ? 'selected' : '' }} @endisset>
                    ...
                    <select
                        ...
                        @foreach ($size as $size)
                            <option
                                class=" bg-gray-200 text-black-500 hover:bg-blue-500 hover:text-white cursor-pointer my-2 p-3 focus:bg-blue-700 "
                                value="{{ $size['size_id'] }}"
                                @isset($old_size_name){{ $old_size_name === $size['size_name'] ? 'selected' : '' }} @endisset>
            ...
            @if ($errors->has('err'))
                <x-input label="enter size price" id="price" type="text" name="price"
                    placeholder="enter size price" valuetext="{{ old('price') }}" redClass="border-red-500" />
            @else
                <x-input label="enter size price" id="price" type="text" name="price"
                    placeholder="enter size price" valuetext="{{ isset($old_price) ? $old_price : '' }}"
                ...
                <div class=" flex items-center justify-between" id="showsubcategorys">
                    <div class="flex items-center">
                        ...
                        <div class="flex flex-col ml-3">
                            ...
                            <div class="font-medium  leading-none my-2">Tier:
                                {{ $cake['tier_name'] }}
                            </div>
                            <div class="font-medium  leading-none">size:
                                {{ $cake['size_name'] }}
                            </div>
                            <div class="font-medium leading-none text-red-500 my-2">Price : {{ $cake['price'] }}
                            </div>
                            </p>
                        </div>
                    </div>
