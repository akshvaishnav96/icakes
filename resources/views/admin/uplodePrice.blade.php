...
@section('main-section')
    <div class="mx-auto w-full max-w-[550px] bg-white my-5">
        ...
        <form class="py-6 px-9" action="{{ $url }}" method="post">
            @csrf
            @if ($errors->has('err'))
                <x-input label="enter price name" id="cake_prices" type="number" name="price_value"
                    placeholder="enter pricename" valuetext="{{ old('pricename') }}" redClass="border-red-500" />
            @else
                <x-input label="enter price name" id="cake_prices" type="number" name="price_value"
                    placeholder="enter pricename" valuetext="{{ isset($oldprice) ? $oldprice : '' }}"
        ...
        @foreach ($price as $key => $price)
            <div
                ...
                <div class=" flex items-center justify-between" id="showprices">
                    <div class="flex items-center">

                        <div class="flex flex-col ml-3">
                            <a class="text-xl font-semibold text-gray-800"></a>
                            <div class="font-medium leading-none">{{ $price->price_value }} price</div>
                            </p>
                        </div>
                    </div>
                    <a href ="/price/edit/{{ $price->price_id }}"><button id ="uplodepriceEditBtn"
                    ...
                    <a href = "/price/{{ $price->price_id }}"> <button id ="uplodepriceDelete Btn"
