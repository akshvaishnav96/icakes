...
@section('main-section')
    <div class="mx-auto w-full max-w-[550px] bg-white my-5">
        ...
        <form class="py-6 px-9" action="{{ $url }}" method="post">
            @csrf
            @if ($errors->has('err'))
                <x-input label="enter tier name" id="cake_tiers" type="number" name="tier_name" placeholder="enter tiername"
                    valuetext="{{ old('tiername') }}" redClass="border-red-500" />
            @else
                <x-input label="enter tier name" id="cake_tiers" type="number" name="tier_name"
                    placeholder="enter tiername" valuetext="{{ isset($oldtier) ? $oldtier : '' }}"
        ...
        @foreach ($tier as $key => $tier)
            <div
                ...
                <div class=" flex items-center justify-between" id="showtiers">
                    <div class="flex items-center">

                        <div class="flex flex-col ml-3">
                            <a class="text-xl font-semibold text-gray-800"></a>
                            <div class="font-medium leading-none">{{ $tier->tier_name }} tier</div>
                            </p>
                        </div>
                    </div>
                    <a href ="/tier/edit/{{ $tier->tier_id }}"><button id ="uplodetierEditBtn"
                    ...
                    <a href = "/tier/{{ $tier->tier_id }}"> <button id ="uplodetierDelete Btn"
